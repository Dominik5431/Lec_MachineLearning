...
# load datasets
data = [[], [], []]
data[0] = np.loadtxt(r'data1')
data[1] = np.loadtxt(r'data2')
data[2] = np.loadtxt(r'data3')
...
# load gamma values
testgamma = [[], [], []]
testgamma[0] = np.loadtxt(r'gamma1')
testgamma[1] = np.loadtxt(r'gamma2')
testgamma[2] = np.loadtxt(r'gamma3')
for idx in range(3):
...
sdata = np.loadtxt(r'skin.dat')
ndata = np.loadtxt(r'non-skin.dat')



img = im2double(imread(r'faces.png'))

#%%
print('Now skin detection function is called.')
skin = skinDetection(ndata, sdata, skin_K, skin_n_iter, skin_epsilon, theta, img)
plt.imshow(skin)
plt.show()
imsave(r'C:\Users\Dominik\Documents\Uni-Dokumente\Master\2. Semester\Machine Learning\exercise-01\q6_expectation_maximization_python\skin_detection.png', skin)
